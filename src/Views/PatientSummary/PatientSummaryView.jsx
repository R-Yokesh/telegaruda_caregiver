...
function PatientSummaryView() {
  ...
  const consultData = location.state.consultDetail;
  const goTo = (
    PatientMenu,
    PatientSubMenu1,
    PatientSubMenu2,
  ...
  const getConsultSummary = useCallback(async () => {
    const getPublisherData = consultData?.participants?.filter(
      (participant) => participant?.role === "publisher"
    );
    try {
      const response = await get(
        `patient/summary?token=${getPublisherData[0]?.token}&pdf_for=consult_report`
      );
  ...
  return (
    <section className="patient-summary-sec">
      <div className="detailing-card">
        <PatientDetailCard data={summaryData?.profile} />
      </div>
              ...
              {summaryData?.health?.a_chief_complaints?.length >= 0 && (
                <div className="mb-2">
                  ...
                  <ChiefComplaints
                    ...
                    consultSummaryData={summaryData?.health?.a_chief_complaints}
                  />
                </div>
              )}
